...
class GattCharacteristic:
    ...
    def add_descriptor(self, uuid, read_perm=0, write_perm=0, comments=''):
    ...
    @staticmethod
    def perm2str(perm):
        if perm == GattCharacteristic.PERM_SC:
            return 'sc'
        elif perm == GattCharacteristic.PERM_AUTH:
            return 'auth'
        elif perm == GattCharacteristic.PERM_AUTH:
            return 'un_auth'
        else:
    ...
    def str2perm(perm_str):
        if perm_str == 'sc':
            return GattCharacteristic.PERM_SC
        elif perm_str == 'auth':
            return GattCharacteristic.PERM_AUTH
        elif perm_str == 'un_auth':
            return GattCharacteristic.PERM_UN_AUTH
        else:
            return GattCharacteristic.PERM_NO_AUTH
                ...
                if isinstance(serv_characteristics, list):
                    for characteristic in serv_characteristics:
                        ...
                        add_char = add_service.add_characteristic(
                            char_name, char_uuid, GattCharacteristic.str2prop(char_properties), char_max_length,
                            GattCharacteristic.str2perm(read_permission), GattCharacteristic.str2perm(write_permission),
                            char_comments)
                        # values
                        char_values = characteristic.get('values', None)
                        if isinstance(char_values, list) and len(char_values) > 0:
                        ...
                        if isinstance(char_descriptors, list) and len(char_descriptors) > 0:
                            for desc in char_descriptors:
                                ...
                                if isinstance(desc_permission, dict):
                                    ...
                                    write_permission = desc.get('write', 'no_auth')
                                desc_comments = desc.get('comments', '')
                                add_char.add_descriptor(desc_uuid, GattDescriptor.str2perm(read_permission),
                                                        GattDescriptor.str2perm(write_permission), desc_comments)
            else:
...
if __name__ == '__main__':
    ...
    print(str(GattServices('gatt_services.json')))
